...
def main():
    ...
    scaler = GradScaler('cuda', enabled=args.use_amp, init_scale=1.0, growth_factor=1.01, backoff_factor=0.5, growth_interval=1000)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for cur_iter, batch in enumerate(train_dl):
            ...
            with autocast('cuda', enabled=args.use_amp):
                ...
                inout_probs = torch.stack(preds['inout']).squeeze(dim=1)
            ...
            logits_inout = inout_logits.float()
            target_inout = inout_cuda.float()
            if args.use_amp:
                with autocast('cuda', enabled=False):
                    inout_loss = inout_loss_fn(logits_inout, target_inout)
            else:
                inout_loss = inout_loss_fn(logits_inout, target_inout)
