...
class DinoV3Backbone(Backbone):
    ...
    def forward(self, x):
        ...
        if isinstance(features, list):
            # some DINOV3 implementations return a list when multiple crops are processed
        ...
        if isinstance(features, dict):
            if 'features' in features:
                ...
                x = tokens.transpose(1, 2).contiguous().reshape(b, dim, out_h, out_w)
            else:
                raise KeyError("Unexpected keys returned from DINOv3 backbone: {}".format(features.keys()))
        else:
            raise TypeError("Unexpected output type from DINOv3 backbone: {}".format(type(features)))
    ...
    def get_out_size(self, in_size):
        h, w = in_size
        return (h // self.dinov3.patch_size, w // self.dinov3.patch_size)
